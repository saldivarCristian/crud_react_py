...
//acceso al sistema por app
    $app->post('/cliente/user/st/login2', function(Request $request, Response $response){
...
//Add usuario
//Add usuario
$app->post('/cliente/user/st/addUsuario2', function(Request $request, Response $response){
    ...
    $ciudadCliente = $request->getParam('id_ciudad');
    $barrioCliente = $request->getParam('id_barrio');
    $password = $request->getParam('password');
    ...
    $sqlNumeroTelefono = "SELECT COUNT(*) AS cantidad FROM  ".DB_CON.".calautos_clientes WHERE tel_con_cliente = '$telefonoCliente'";
    ...
    $sqlrucCliente = "SELECT COUNT(*) AS cantidad FROM  ".DB_CON.".calautos_clientes WHERE ruc_cliente = '$rucCliente'";
    ...
    // $sqlValidacionNickname = "SELECT id_cliente FROM ".DB_CON.".calautos_clientes WHERE nickname = '$nickname'";
    ...
    try{
        $sql = "INSERT INTO ".DB_CON.".calautos_clientes 
        (
            razon_cliente,
            cliente_cliente,
            socio_cliente,
        ...
        $stmt->bindParam(':sexo_cliente', $sexoCliente);
        ...
        $stmt->bindParam(':id_ciudad', $ciudadCliente);
        $stmt->bindParam(':id_barrio', $barrioCliente);
...
//Actualizar datos personales
    $app->post('/cliente/user/editUsuario2', function(Request $request, Response $response){
        ...
        if (@$token['aud'] === Aud())
        {
            try
            {
                ...
                $sql_nro = "SELECT COUNT(*) AS cantidad FROM  ".DB_CON.".calautos_clientes WHERE tel_con_cliente = '$telefono' AND ruc_cliente != '$ci_usu' ";
                ...
                $sqlValidacionNickname = "SELECT id_cliente FROM ".DB_CON.".calautos_clientes WHERE nickname = '$nickname' AND id_cliente != $idCliente";
                $stmt = $db->query($sqlValidacionNickname);
                $datosNickname = $stmt->fetch(PDO::FETCH_OBJ);
                $count = $stmt->rowCount();
                if($count > 0)
                ...
                $sql = "SELECT estado_cliente FROM ".DB_CON.".calautos_clientes WHERE id_cliente =".$idCliente;
                ...
                {
                    $sql = "UPDATE ".DB_CON.".calautos_clientes SET
...
//RECUPERAR CONTRASENA
    $app->post('/cliente/user/st/recuperar2', function(Request $request, Response $response){
        try
        {
            $db = $this->db;
            ...
            $sql = "UPDATE ".DB_CON.".calautos_clientes SET pass_cliente='".$pass_usu."' where email_con_cliente='".$email."'";
            $stmt = $db->prepare($sql);
            $stmt->execute();
...
//Cambiar Contrasena
    $app->post('/cliente/user/cambiarContrasena2', function(Request $request, Response $response){
        ...
        if (@$token['aud'] === Aud())
        {
            try
            {
                ...
                // Get DB Object
                $db = $this->db;
                $sql_usuario = "SELECT * FROM  ".DB_CON.".calautos_clientes WHERE id_cliente = '$id_cliente'";
                $stmt = $db->query($sql_usuario);
                $datos = $stmt->fetch(PDO::FETCH_OBJ);

                if(password_verify($password_viejo, @$datos->pass_cliente) ){
                    $sql = "UPDATE ".DB_CON.".calautos_clientes SET pass_cliente = '$pass_usu' WHERE id_cliente = '$id_cliente'";
                    $stmt = $db->prepare($sql);
                    $stmt->execute();
                    return $response->withJson(
    ...
    $app->get('/register/listSelect', function(Request $request, Response $response, array $args){
        try{
            $db = $this->db;
            $listbarrios = \Clases\Admin\General\Usuarios::listarBarrios($db);
            $listciudades = \Clases\Admin\General\Usuarios::listarCiudades($db);
            return $this->response->withJson([
                'code' => 200,
                'status' => 'success', 
                'message' => '',
                'data' => [
                   "ciudades"=>$listciudades,"barrios"=>$listbarrios
                ]
            ]);
        } catch(PDOException $e)
        {
