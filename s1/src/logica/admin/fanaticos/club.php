...
use \Clases\Admin\Fanaticos\Clubes;

$app->group('/admin/clubes', function(\Slim\App $app) {
    //Listar
    ...
    $app->post('/add', function(Request $request, Response $response, array $args){
        try{
            ...
            $newObj = [    
                "nombre_club" => $parametros['nombre_club'],
                "ubicacion_club" => $parametros['ubicacion_club'],
                "id_pais" => $parametros['id_pais'],
                "fecha_creador" => date("Y-m-d H:y:s"),

            ];
            $idClub = $claseClub->insertar($db,$newObj);

            $target = 'fanaticos';
            $responseImage = uploadFile('data', $target, 'clubes');
            ...
            if( isset( $responseImage['error'] ) ){
                ...
                $claseClub->actualizar($db,$newObj,$idClub);
            }
    ...
    $app->get('/edit/{id}', function($request, $response, $args){
        try{
            ...
            $id = $args['id'];
            $datos = Clubes::getDataById($db ,$id);
            if($datos){
                return $this->response->withJson([
                    'code' => 200,
                    'status' => 'success', 
    ...
    $app->post('/update/{id}', function($request, $response, $args){
        try{
            $db = $this->db;
            $claseClub = new Clubes( $this->logger );
            $parametros = $request->getParsedBody();
            $parametros = $parametros['data'];
            if(!count($parametros)){
                return $this->response->withJson([
            ...
            $id = $args['id'];
            $idClub = $claseClub->actualizar($db,$newObj,$id);
            if (!empty($_FILES['data'])) {
                ...
                if( isset( $responseImage['error'] ) ){
                    ...
                    $claseClub->actualizar($db,$newObj,$id);
                    unlink(__DIR__."/../../../../public/img/fanaticos/lg/clubes/".$parametros['file_hidden']);
                    unlink(__DIR__."/../../../../public/img/fanaticos/sm/clubes/".$parametros['file_hidden']);
                    unlink(__DIR__."/../../../../public/img/fanaticos/md/clubes/".$parametros['file_hidden']);
