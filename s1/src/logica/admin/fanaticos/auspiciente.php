...
use \Clases\Admin\Fanaticos\Auspiciantes;

$app->group('/admin/auspiciantes', function(\Slim\App $app) {
    //Listar
    $app->get('/list', function(Request $request, Response $response, array $args){
        try{
            $db = $this->db;
            $list = Auspiciantes::listar($db);
    ...
    $app->post('/add', function(Request $request, Response $response, array $args){
        try{
            $db = $this->db;
            $claseAuspiciante = new Auspiciantes;
    ...
    $app->get('/edit/{id}', function($request, $response, $args){
        try{
            ...
            $id = $args['id'];
            $datos = Auspiciantes::getDataById($db ,$id);
            if($datos){
                return $this->response->withJson([
                    'code' => 200,
                    'status' => 'success', 
    ...
    $app->post('/update/{id}', function($request, $response, $args){
        try{
            $db = $this->db;
            $claseAuspiciante = new Auspiciantes( $this->logger );
            $parametros = $request->getParsedBody();
            $parametros = $parametros['data'];
            if(!count($parametros)){
                return $this->response->withJson([
