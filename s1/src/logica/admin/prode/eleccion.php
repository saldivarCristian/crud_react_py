...
$app->group('/admin/prode/eleccion', function(\Slim\App $app) {
    //Listar
    ...
    $app->get('/listSelectById/{id}', function(Request $request, Response $response, array $args){
        try{
            $db = $this->db;
            $eleccion = \Clases\Admin\Prode\Elecciones::getDataById($db,$args['id']);
            $listPoliticos = \Clases\Admin\Prode\Politicos::listarPorCargo($db,$args['id']);
            $seleccionPoliticos = \Clases\Admin\Prode\Elecciones::listFormacionById($db,$args['id']);
            return $this->response->withJson([
                'code' => 200,
                'status' => 'success', 
                'message' => '',
...
//RUTAS DE CARGOS
    ...
    $app->get('/eleccionPostulantes/listSelectById/{id}', function (Request $request, Response $response, array $args) {
        try {
            $db = $this->db;
            $seleccionCargo = \Clases\Admin\Prode\Elecciones::listarPostulantesCargo($db, $args['id']);
            $listPoliticos = \Clases\Admin\Prode\Elecciones::listarPoliticosCargo($db, $args['id']);
            // $seleccionPoliticos = \Clases\Admin\Prode\Elecciones::listFormacionById($db,$args['id']);
            return $this->response->withJson([
                'code' => 200,
                'status' => 'success',
