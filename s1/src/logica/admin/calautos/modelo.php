...
use \Clases\Admin\Calautos\Modelos;
use \Clases\Admin\Restricciones;

$app->group('/admin/calautos/modelo', function(\Slim\App $app) {
    //Listar
    $app->get('/list', function(Request $request, Response $response, array $args){

        try{
            $db = $this->db;
            $list = Modelos::listar($db);
    ...
    $app->get('/listSelect', function(Request $request, Response $response, array $args){
        try{
            ...
            $listMarca = \Clases\Admin\Calautos\Marcas::listar($db);
            return $this->response->withJson([
                'code' => 200,
                'status' => 'success', 
                'message' => '',
                'data' => [
                   "estados"=>$listEstados,"marcas"=>$listMarca
                ]
            ]);
        } catch(PDOException $e)
        {
    ...
    $app->post('/add', function(Request $request, Response $response, array $args){
        try{
            ...
            $newObj = [
                ...
                "nombre_modelo" => $parametros['nombre_modelo'],
                "estado_modelo" => $parametros['estado_modelo'],
                "insert_local" => $parametros['insert_local'],
                "id_creador" => $parametros['id_creador'],
                "fecha_creador" => date("Y-m-d H:y:s"),

            ];
            $idModelos = $claseModelos->insertar($db,$newObj);
    ...
    $app->get('/edit/{id}', function($request, $response, $args){
        try{
            ...
            $id = $args['id'];
            $datos = Modelos::getDataById($db ,$id);
            if($datos){
                return $this->response->withJson([
                    'code' => 200,
                    'status' => 'success', 
    ...
    $app->post('/update/{id}', function($request, $response, $args){
        try{
            $db = $this->db;
            $claseSeleccion = new Modelos( $this->logger );
            $parametros = $request->getParsedBody();
            if(!count($parametros)){
                return $this->response->withJson([
                    'code' => 100,
