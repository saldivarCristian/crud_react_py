...
$app->group('/admin/calautos/auspiciantes1', function(\Slim\App $app) {
    //Listar
    $app->get('/list', function(Request $request, Response $response, array $args){
        try{
            $db = $this->db;
            $list = Auspiciantes1::listar($db);
    ...
    $app->post('/add', function(Request $request, Response $response, array $args){
        try{
            $db = $this->db;
            $claseAuspiciante = new Auspiciantes1;
    ...
    $app->get('/edit/{id}', function($request, $response, $args){
        try{
            ...
            $id = $args['id'];
            $datos = Auspiciantes1::getDataById($db ,$id);
            if($datos){
                return $this->response->withJson([
                    'code' => 200,
                    'status' => 'success', 
    ...
    $app->post('/update/{id}', function($request, $response, $args){
        try{
            $db = $this->db;
            $claseAuspiciante = new Auspiciantes1( $this->logger );
            $parametros = $request->getParsedBody();
            $parametros = $parametros['data'];
            if(!count($parametros)){
                return $this->response->withJson([
    ...
    $app->delete('/delete/{id}', function(Request $request, Response $response, array $args){
        try{
            ...
            $id = $args['id'];
            $claseUsuario = new Auspiciantes1;
