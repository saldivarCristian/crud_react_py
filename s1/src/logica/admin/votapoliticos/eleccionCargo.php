...
use \Clases\Admin\Votapoliticos\EleccionesCargos;

$app->group('/admin/votapoliticos/eleccionCargo', function(\Slim\App $app) {
    //Listar
    ...
    $app->get('/listSelectById/{id}', function(Request $request, Response $response, array $args){
        try{
            $db = $this->db;
            $eleccionCargo = \Clases\Admin\Votapoliticos\EleccionesCargos::getDataById($db,$args['id']);
            $listPoliticos = \Clases\Admin\Votapoliticos\Politicos::listarPorCargo($db,$args['id']);
            $seleccionPoliticos = \Clases\Admin\Votapoliticos\EleccionesCargos::listFormacionById($db,$args['id']);
            return $this->response->withJson([
                'code' => 200,
                'status' => 'success', 
                'message' => '',
