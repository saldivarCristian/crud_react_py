...
use \Clases\Admin\Votapoliticos\Cargos;
use \Clases\Admin\Restricciones;

$app->group('/admin/votapoliticos/cargo', function(\Slim\App $app) {
    //Listar
    $app->get('/list', function(Request $request, Response $response, array $args){

        try{
            $db = $this->db;
            $list = Cargos::listar($db);
    ...
    $app->post('/add', function(Request $request, Response $response, array $args){
        try{
            ...
            $newObj = [ 
                "nombre_cargo" => $parametros['nombre_cargo'],
                "estado_cargo" => $parametros['estado_cargo'],
                //"insert_local" => $parametros['insert_local'],
                "id_creador" => $parametros['id_creador'],
                "fecha_creador" => date("Y-m-d H:y:s"),

            ];
            $idCargos = $claseCargos->insertar($db,$newObj);
    ...
    $app->get('/edit/{id}', function($request, $response, $args){
        try{
            ...
            $id = $args['id'];
            $datos = Cargos::getDataById($db ,$id);
            if($datos){
                return $this->response->withJson([
                    'code' => 200,
                    'status' => 'success', 
    ...
    $app->post('/update/{id}', function($request, $response, $args){
        try{
            $db = $this->db;
            $claseSeleccion = new Cargos( $this->logger );
            $parametros = $request->getParsedBody();
            if(!count($parametros)){
                return $this->response->withJson([
                    'code' => 100,
