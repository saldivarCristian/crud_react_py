...
use \Clases\Admin\Desafios\Estadios;

$app->group('/admin/desafios/stadiums', function(\Slim\App $app) {
    //Listar
    ...
    $app->post('/add', function(Request $request, Response $response, array $args){
        try{
            ...
            $newObj = [    
                "capacidad_estadio"=> $parametros['capacidad_estadio'],
                "id_club"=> $parametros['id_club'],
                "nombre_estadio"=> $parametros['nombre_estadio'],
    ...
    $app->get('/edit/{id}', function($request, $response, $args){
        try{
            ...
            $id = $args['id'];
            $datos = Estadios::getDataById($db ,$id);
            if($datos){
                return $this->response->withJson([
                    'code' => 200,
                    'status' => 'success', 
    ...
    $app->post('/update/{id}', function($request, $response, $args){
        try{
            $db = $this->db;
            $claseUsuario = new Estadios( $this->logger );
            $parametros = $request->getParsedBody();
            if(!count($parametros)){
                return $this->response->withJson([
                    'code' => 100,
