...
class Marca {
    ...
    public static function listarPorEstado(pdo $db, $verComlumnas=[]): array
    {
        ...
        $sql = "SELECT $columna FROM ".DB_CON.".calautos_marcas WHERE estado_auspiciante = 0";
        $stmt = $db->query($sql);
        $data = $stmt->fetchAll(PDO::FETCH_OBJ);
        return $data;
    }
    ...
    public static function getDataByNombre($db, $id)
    {
        $sqlCliente = "SELECT
                   *
                FROM ".DB_CON.".calautos_marcas WHERE id_auspiciante = '$id'";
    ...
    public static function getDataByCuil($db, $nombre_auspiciante)
    {
        $sql = "SELECT
                   *
                FROM ".DB_CON.".calautos_marcas WHERE id_auspiciante = '$nombre_auspiciante'";
        $stmt = $db->query($sql);
        $data = $stmt->fetch(PDO::FETCH_OBJ);
        return $data;
    }
    ...
    {        
        $sql = "
            DELETE FROM ".DB_CON.".calautos_marcas where id_auspiciante = $id_update
        ";
        $stmt = $db->prepare($sql);
        $stmt->execute();
        $id = $db->lastInsertId();
    ...
    public static function getDataById(pdo $db, $id_auspiciante, $verComlumnas=[]): object
    {
        ...
        $sql = "SELECT $columna FROM ".DB_CON.".calautos_marcas WHERE id_auspiciante = $id_auspiciante";
        $stmt = $db->query($sql);
        $data = $stmt->fetch(PDO::FETCH_OBJ);
        return $data;
    }
