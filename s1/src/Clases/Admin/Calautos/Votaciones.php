...
class Votaciones {
    ...
    {        
        $sql = "
            DELETE FROM ".DB_CON.".calautos_votaciones
        ";
        $stmt = $db->prepare($sql);
        $stmt->execute();
        return [];
    ...
    public static function getInformesConFiltro(pdo $db, $id = "" )
    {
        $where = "";
        if ($id =="mujeres" ) {
            $where .= " and c.sexo_cliente ='f' ";
        }
        if ($id =="hombres" ) {
            $where .= " and c.sexo_cliente ='m' ";
        }
        if ($id =="socios" ) {
            $where .= " and c.socio_cliente ='1' ";
        }
        if ($id =="nosocios" ) {
            $where .= " and c.socio_cliente ='2' ";
        }
        ...
        COALESCE( ( 100 * ( SUM(v.calificacion_votacion)) / COUNT(v.id_auto) ) / 10 ,0 )  porcentaje
            ...
            JOIN ".DB_BASE.".calautos_clientes c ON c.id_cliente = v.id_cliente $where
   ...
   FROM ".DB_BASE.".calautos_votaciones v
   JOIN ".DB_BASE.".calautos_clientes c ON c.id_cliente = v.id_cliente
    
    ";
    $stmt = $db->query($sql);
    $data2 = $stmt->fetchAll(PDO::FETCH_OBJ);
    return ["detalle1"=>$data,"detalle2"=>$data2];
}
    ...
    public static function getListCalification2(pdo $db, $idCalificador="", $idCliente = "" )
    {
        $where = "";
        if ($idCalificador !="" ) {
