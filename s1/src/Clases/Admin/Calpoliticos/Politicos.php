...
class Autos {
    ...
    public static function listarPorEstado(pdo $db, $verComlumnas=[]): array
    {
        ...
        $sql = "SELECT $columna FROM ".DB_CON.".calautos_autos WHERE estado_auto = 0";
        $stmt = $db->query($sql);
        $data = $stmt->fetchAll(PDO::FETCH_OBJ);
        return $data;
    }
     ...
     *
     * @param [type] $db conexion de base de datos
     * @param [type] $id_auto filtar autos por id_auto
     * @return void
     */
    public static function getDataByNombre($db, $id)
    {
        $sqlCliente = "SELECT
                   *
                FROM ".DB_CON.".calautos_autos WHERE id_auto = '$id'";
     ...
     * @return void
     */
    public static function getDataByCuil($db, $id_auto)
    {
        $sql = "SELECT
                   *
                FROM ".DB_CON.".calautos_autos WHERE id_auto = '$id_auto'";
        $stmt = $db->query($sql);
        $data = $stmt->fetch(PDO::FETCH_OBJ);
        return $data;
    }
    ...
    {        
        $sql = "
            DELETE FROM ".DB_CON.".calautos_autos where id_auto = $id_update
        ";
        $stmt = $db->prepare($sql);
        $stmt->execute();
        $id = $db->lastInsertId();
     ...
     * @return array
     */
    public static function getDataById(pdo $db, $id_auto, $verComlumnas=[]): object
    {
        $columna = "*";
        if (count($verComlumnas)) {
            $columna = implode(",", $verComlumnas);
        }
        $sql = "SELECT $columna FROM ".DB_CON.".calautos_autos WHERE id_auto = $id_auto";
        $stmt = $db->query($sql);
        $data = $stmt->fetch(PDO::FETCH_OBJ);
        return $data;
    }
