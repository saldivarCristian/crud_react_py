...
class Clientes {
    ...
    public static function listar(pdo $db, $verComlumnas=[]): array
    {
        ...
        $sql = "SELECT $columna FROM ".DB_CON.".calautos_clientes";
        $stmt = $db->query($sql);
    ...
    public static function getDataByCI($db, $ci)
    {
        $sqlCliente = "SELECT
                   *
                FROM ".DB_CON.".calautos_clientes WHERE ruc_cliente = '$ci'";
    ...
    public static function getDataByCuil($db, $cuil)
    {
        $sql = "SELECT
                   *
                FROM ".DB_CON.".calautos_clientes WHERE ruc_cliente = '$cuil'";
        $stmt = $db->query($sql);
        $data = $stmt->fetch(PDO::FETCH_OBJ);
        return $data;
    }
    ...
    public function insertar($db,$obj = [])
    {
        ...
        $sql = "
            INSERT INTO ".DB_CON.".calautos_clientes (
                $columnaAInsertar
            )
            VALUES (
                $variableAInsertar   
    ...
    public function actualizar($db,$obj = [],$id_update)
    {
        ...
        $sql = "
            update ".DB_CON.".calautos_clientes set
                $columnaActualizar
    ...
    {        
        $sql = "
            DELETE FROM ".DB_CON.".calautos_clientes where id_cliente = $id_update
        ";
        $stmt = $db->prepare($sql);
        $stmt->execute();
        $id = $db->lastInsertId();
    ...
    public static function getDataById(pdo $db, $id_cliente, $verComlumnas=[]): object
    {
        ...
        $sql = "SELECT $columna FROM ".DB_CON.".calautos_clientes WHERE id_cliente = $id_cliente";
        $stmt = $db->query($sql);
        $data = $stmt->fetch(PDO::FETCH_OBJ);
        return $data;
    }
