...
class Torneos {
    ...
    public static function listar(pdo $db, $verComlumnas=[]): array
    {
        ...
        $sql = "SELECT $columna FROM ".DB_CON.".fanatico_torneos";
        $stmt = $db->query($sql);
     ...
     *
     * @param [type] $db conexion de base de datos
     * @param [type] $id_torneo filtar torneos por id_torneo
     * @return void
     */
    public static function getDataByNombre($db, $id)
    {
        $sqlCliente = "SELECT
                   *
                FROM ".DB_CON.".fanatico_torneos WHERE id_torneo = '$id'";
     ...
     *
     * @param [type] $db conexion de base de datos
     * @param [type] $nombre_club filtar torneos por nombre_club
     * @return void
     */
    public static function getDataByCuil($db, $nombre_club)
    {
        $sql = "SELECT
                   *
                FROM ".DB_CON.".fanatico_torneos WHERE id_torneo = '$nombre_club'";
        $stmt = $db->query($sql);
        $data = $stmt->fetch(PDO::FETCH_OBJ);
        return $data;
    }
    ...
    public function insertar($db,$obj = [])
    {
        ...
        $sql = "
            INSERT INTO ".DB_CON.".fanatico_torneos (
                $columnaAInsertar
            )
            VALUES (
                $variableAInsertar   
    ...
    public function actualizar($db,$obj = [],$id_update)
    {
        ...
        $sql = "
            update ".DB_CON.".fanatico_torneos set
                $columnaActualizar
           where id_torneo = $id_update
        ";
    ...
    {        
        $sql = "
            DELETE FROM ".DB_CON.".fanatico_torneos where id_torneo = $id_update
        ";
        $stmt = $db->prepare($sql);
        $stmt->execute();
        $id = $db->lastInsertId();
     ...
     *
     * @param pdo $db
     * @param [type] $id_torneo
     * @param array $verComlumnas
     * @return array
     */
    public static function getDataById(pdo $db, $id_torneo, $verComlumnas=[]): object
    {
        $columna = "*";
        if (count($verComlumnas)) {
            $columna = implode(",", $verComlumnas);
        }
        $sql = "SELECT $columna FROM ".DB_CON.".fanatico_torneos WHERE id_torneo = $id_torneo";
        $stmt = $db->query($sql);
        $data = $stmt->fetch(PDO::FETCH_OBJ);
        return $data;
    }
