...
class Clubes {
    ...
    public static function listar(pdo $db, $verComlumnas=[]): array
    {
        ...
        $sql = "SELECT $columna FROM ".DB_CON.".fanatico_clubes";
        $stmt = $db->query($sql);
     ...
     *
     * @param [type] $db conexion de base de datos
     * @param [type] $id_club filtar clubes por id_club
     * @return void
     */
    public static function getDataByNombre($db, $id)
    {
        $sqlCliente = "SELECT
                   *
                FROM ".DB_CON.".fanatico_clubes WHERE id_club = '$id'";
     ...
     *
     * @param [type] $db conexion de base de datos
     * @param [type] $nombre_club filtar clubes por nombre_club
     * @return void
     */
    public static function getDataByCuil($db, $nombre_club)
    {
        $sql = "SELECT
                   *
                FROM ".DB_CON.".fanatico_clubes WHERE id_club = '$nombre_club'";
        $stmt = $db->query($sql);
        $data = $stmt->fetch(PDO::FETCH_OBJ);
        return $data;
    }
    ...
    public function insertar($db,$obj = [])
    {
        ...
        $sql = "
            INSERT INTO ".DB_CON.".fanatico_clubes (
                $columnaAInsertar
            )
            VALUES (
                $variableAInsertar   
    ...
    public function actualizar($db,$obj = [],$id_update)
    {
        ...
        $sql = "
            update ".DB_CON.".fanatico_clubes set
                $columnaActualizar
           where id_club = $id_update
        ";
    ...
    {        
        $sql = "
            DELETE FROM ".DB_CON.".fanatico_clubes where id_club = $id_update
        ";
        $stmt = $db->prepare($sql);
        $stmt->execute();
        $id = $db->lastInsertId();
     ...
     * @return array
     */
    public static function getDataById(pdo $db, $id_club, $verComlumnas=[]): object
    {
        $columna = "*";
        if (count($verComlumnas)) {
            $columna = implode(",", $verComlumnas);
        }
        $sql = "SELECT $columna FROM ".DB_CON.".fanatico_clubes WHERE id_club = $id_club";
        $stmt = $db->query($sql);
        $data = $stmt->fetch(PDO::FETCH_OBJ);
        return $data;
    }
