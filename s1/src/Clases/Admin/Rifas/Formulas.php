...
class Formulas {
    ...
    public static function listar(pdo $db, $verComlumnas=[]): array
    {
        ...
        $sql = "SELECT $columna FROM ".DB_CON.".rifa_formulas";
        $stmt = $db->query($sql);
     ...
     *
     * @param [type] $db conexion de base de datos
     * @param [type] $nombre_formula filtar formula por nombre_formula
     * @return void
     */
    ...
    public function insertar($db,$obj = [])
    {
        ...
        $sql = "
            INSERT INTO ".DB_CON.".rifa_formulas (
                $columnaAInsertar
            )
            VALUES (
                $variableAInsertar   
    ...
    public function actualizar($db,$obj = [],$id_update)
    {
        ...
        $sql = "
            update ".DB_CON.".rifa_formulas set
                $columnaActualizar
           where id_formula = $id_update
        ";
    ...
    {        
        $sql = "
            DELETE FROM ".DB_CON.".rifa_formulas where id_formula = $id_update
        ";
        $stmt = $db->prepare($sql);
        $stmt->execute();
        $id = $db->lastInsertId();
     ...
     *
     * @param pdo $db
     * @param [type] $id_formula
     * @param array $verComlumnas
     * @return array
     */
    public static function getDataById(pdo $db, $id_formula, $verComlumnas=[]): object
    {
        $columna = "*";
        if (count($verComlumnas)) {
            $columna = implode(",", $verComlumnas);
        }
        $sql = "SELECT $columna FROM ".DB_CON.".rifa_formulas WHERE id_formula = $id_formula";
        $stmt = $db->query($sql);
