...
class Productos {
    ...
    public static function listar(pdo $db, $verComlumnas=[]): array
    {
        ...
        $sql = "SELECT $columna FROM ".DB_CON.".rifa_productos";
        $stmt = $db->query($sql);
     ...
     *
     * @param [type] $db conexion de base de datos
     * @param [type] $nombre_producto filtar formula por nombre_producto
     * @return void
     */
    ...
    public function insertar($db,$obj = [])
    {
        ...
        $sql = "
            INSERT INTO ".DB_CON.".rifa_productos (
                $columnaAInsertar
            )
            VALUES (
                $variableAInsertar   
    ...
    public function actualizar($db,$obj = [],$id_update)
    {
        ...
        $sql = "
            update ".DB_CON.".rifa_productos set
                $columnaActualizar
    ...
    {        
        $sql = "
            DELETE FROM ".DB_CON.".rifa_productos where id_producto = $id_update
        ";
        $stmt = $db->prepare($sql);
        $stmt->execute();
        $id = $db->lastInsertId();
     ...
     * @return array
     */
    public static function getDataById(pdo $db, $id_producto, $verComlumnas=[]): object
    {
        $columna = "*";
        if (count($verComlumnas)) {
            $columna = implode(",", $verComlumnas);
        }
        $sql = "SELECT $columna FROM ".DB_CON.".rifa_productos WHERE id_producto = $id_producto";
        $stmt = $db->query($sql);
