function Spin2WinWheel(idRule="") {
  var elem = document.querySelector('#'+idRule);
  // Get HTML content
  ...
  <style>
    ...
    .wheelOutline${idRule}, .valueContainer${idRule}, .toast, .centerCircle${idRule}{
      pointer-events:none;
    }
  </style>
      <svg class="wheelSVG${idRule}" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMidYMid meet" text-rendering="optimizeSpeed">
        <g class="mainContainer${idRule}">   
          <g class="wheel${idRule}"/>    
        </g> 
        <g class="centerCircle${idRule}" />
        <g class="wheelOutline${idRule}" />
        <g class="pegContainer${idRule}" >
          <path  class="peg${idRule}" fill="#EEEEEE" d="M22.139,0C5.623,0-1.523,15.572,0.269,27.037c3.392,21.707,21.87,42.232,21.87,42.232	s18.478-20.525,21.87-42.232C45.801,15.572,38.623,0,22.139,0z" /> 
        </g>
        <g class="valueContainer${idRule}" />
      </svg>  
  ...
  var xmlns = "http://www.w3.org/2000/svg",
    ...
    thisWheel = this,
    wheelSVG = select('.wheelSVG'+idRule),
    wheel = select('.wheel'+idRule),
    wheelOutline = select('.wheelOutline'+idRule),
    peg = select('.peg'+idRule),
    pegContainer = select('.pegContainer'+idRule),
    mainContainer = select('.mainContainer'+idRule),
    valueContainer = select('.valueContainer'+idRule),
    centerCircle = select('.centerCircle'+idRule),
    toast = select('.toast'),
    ...
    spinVelocityTracker,
    gameResultsArray = [],
    pegSnd = new Audio('./assets/plugins/winwheel2/media/wheel_tick.mp3'),
    ...
    addValues = function() {
      ...
      for (var i = 0; i < numSegments; i++) {
        ...
        if (segmentValuesArray[i].type == "image") {
          ...
          ig.setAttributeNS(null, 'x', centerX - (25 +wheelImageSize / 2));
          ig.setAttributeNS(null, 'y', 50 +centerY - wheelRadius + wheelImageOffsetY);
          ig.setAttributeNS(null, 'width', 2 * wheelImageSize);
          ig.setAttributeNS(null, 'height',   2 * wheelImageSize);
          ...
          t.setAttributeNS(null, 'y', 20+centerY - wheelRadius + wheelTextOffsetY);
